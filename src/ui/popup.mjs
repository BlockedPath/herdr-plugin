...
rl.on("line", async (line) => {
	...
	if (v === "s") {
		try {
			...
			process.stdout.write(`\x1b_Gf=100,a=T,t=d,s=${800},v=${600};${b64}\x1b\\\n`);
		...
		for (let row = 0; row < 7; row++) {
			for (let col = 0; col < 5; col++) {
				if ((glyph[row] >> (4 - col)) & 1) {
					for (let dy = 0; dy < scale; dy++) for (let dx = 0; dx < scale; dx++) setPixel(x + col * scale + dx, y + row * scale + dy, fg);
				}
			}
		}
	}
...
function createPng(width, height, raw) {
	...
	return Buffer.concat([pngHeader(), chunk("IHDR", ihdr), chunk("IDAT", idat), chunk("IEND", Buffer.alloc(0))]);
}
