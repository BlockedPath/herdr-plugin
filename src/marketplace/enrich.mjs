...
export async function enrichWithMarketplace(receipt, options = {}) {
	...
	try {
		if (offline) {
			...
			if (!cached) throw new Error("marketplace cache unavailable in offline mode");
		...
		if (candidateId) {
			...
			const candidateRepo = receipt.subject.source?.owner && receipt.subject.source?.repo
				? `${receipt.subject.source.owner}/${receipt.subject.source.repo}`
				: null;
			const otherCollisions = collisions.filter((c) => c.fullName !== candidateRepo);
	...
	const findings = collisionFinding ? [...receipt.findings, collisionFinding].sort((a,b)=>a.id.localeCompare(b.id)) : receipt.findings;
	const dimensions = { ...receipt.completeness.dimensions, marketplace: dimension };
	// Top-level complete stays as before unless check === "on" and marketplace not complete
	const coreComplete = Object.entries(dimensions).filter(([k])=>!["marketplace","comparison"].includes(k)).every(([,v])=>v.status==="complete") && !receipt.completeness.unstable;
	...
	const enriched = { ...receipt, findings, completeness: { ...receipt.completeness, complete: topComplete, dimensions }, summary };
